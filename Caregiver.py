...
class Caregiver:
    def __init__(self, CUsername, password=None, salt=None, hash=None):
    ...
    def get(self):
        ...
        get_caregiver_details = "SELECT Salt, Hash FROM Caregivers WHERE CUsername = %s"
        try:
    ...
    def search_caregiver_schedule(self, d):
        ...
        add_availability = "SELECT * FROM Availabilities WHERE TIME= %TIME "
        try:
            cursor.execute(add_availability, (d, self.username))
            # you must call commit() to persist your data if you don't set autocommit to True
            conn.commit()
    ...
    def update_caregiver_schedule(self, d):
        cm = ConnectionManager()
        conn = cm.create_connection()
        cursor = conn.cursor()
        update_availability = "UPDATE Availabilities SET d = NULL WHERE TIME= %TIME "
        try:
            cursor.execute(update_availability, (d, self.username))
            # you must call commit() to persist your data if you don't set autocommit to True
            conn.commit()
        except pymssql.Error:
            print("Error occurred when updating caregiver availability")
